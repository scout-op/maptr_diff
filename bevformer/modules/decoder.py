...
class CustomMSDeformableAttention(BaseModule):
    ...
    def forward(self,
        ...
        """
        # Ensure module parameters are on the same device as query
        self.to(query.device)

        # Ensure main tensors are on the same device as module parameters
        device = self.sampling_offsets.weight.device
        if query.device != device:
            query = query.to(device)
        if value is not None and value.device != device:
            value = value.to(device)
        if query_pos is not None and query_pos.device != device:
            query_pos = query_pos.to(device)
        if reference_points is not None and reference_points.device != device:
            reference_points = reference_points.to(device)
        if spatial_shapes is not None and spatial_shapes.device != device:
            spatial_shapes = spatial_shapes.to(device)
        if level_start_index is not None and level_start_index.device != device:
