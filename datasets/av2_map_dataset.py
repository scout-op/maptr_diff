...
class CustomAV2LocalMapDataset(CustomNuScenesDataset):
    ...
    def union2one(self, queue):
        ...
        for each in queue:
            img_val = each['img']
            if hasattr(img_val, 'data'):
                img_val = img_val.data
            imgs_list.append(img_val)
        metas_map = {}
        prev_pos = None
        prev_angle = None
        for i, each in enumerate(queue):
            meta_val = each['img_metas']
            if hasattr(meta_val, 'data'):
                meta_val = meta_val.data
            metas_map[i] = meta_val
            if i == 0:
    ...
    def _format_gt(self):
        ...
        if (not os.path.exists(self.map_ann_file)) :
            ...
            for sample_id in range(dataset_length):
                ...
                gt_labels_val = gt_sample_dict['gt_labels_3d']
                if hasattr(gt_labels_val, 'data'):
                    gt_labels_val = gt_labels_val.data
                gt_labels = gt_labels_val.numpy() if hasattr(gt_labels_val, 'numpy') else np.array(gt_labels_val)
                gt_bboxes_val = gt_sample_dict['gt_bboxes_3d']
                if hasattr(gt_bboxes_val, 'data'):
                    gt_bboxes_val = gt_bboxes_val.data
                gt_vecs = gt_bboxes_val.instance_list if hasattr(gt_bboxes_val, 'instance_list') else gt_bboxes_val
                # import pdb;pdb.set_trace()
                gt_vec_list = []
    ...
    def to_gt_vectors(self,
                      gt_dict):
        # import pdb;pdb.set_trace()
        gt_labels_val = gt_dict['gt_labels_3d']
        if hasattr(gt_labels_val, 'data'):
            gt_labels_val = gt_labels_val.data
        gt_labels = gt_labels_val
        gt_bboxes_val = gt_dict['gt_bboxes_3d']
        if hasattr(gt_bboxes_val, 'data'):
            gt_bboxes_val = gt_bboxes_val.data
        gt_instances = gt_bboxes_val.instance_list if hasattr(gt_bboxes_val, 'instance_list') else gt_bboxes_val
