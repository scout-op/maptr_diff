...
# DataContainer compatibility for mmcv 2.x
class DC:
    """Simple DataContainer wrapper for backward compatibility."""
    def __init__(self, data, cpu_only=False, stack=False, pad_dims=None):
        self._data = data
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def __init__(self,
        ...
        # Force full initialization to ensure data_list is loaded
        if hasattr(self, 'full_init') and callable(self.full_init):
            self.full_init()
    ...
    @property
    def data_infos(self):
        """Compatibility: MMEngine 2.0 uses data_list instead of data_infos."""
        return self.data_list
    
    def pre_pipeline(self, results):
        """Prepare data dict before entering the data pipeline."""
    ...
    def parse_ann_info(self, info):
        """Parse bbox and label annotations from info dict (handles old format).
        
        Old NuScenes format uses 'gt_boxes', 'gt_names', 'gt_velocity', 'num_lidar_pts', 'num_radar_pts'
        instead of the new 'instances' format.
        """
        import numpy as np
        from mmdet3d.structures import LiDARInstance3DBoxes
        
        # Handle old format pkl files
        if 'gt_boxes' in info and 'gt_names' in info:
            ...
            for name in gt_names:
                if name in self.metainfo['classes']:
                    gt_labels_3d.append(self.metainfo['classes'].index(name))
                else:
                    gt_labels_3d.append(-1)
            gt_labels_3d = np.array(gt_labels_3d, dtype=np.int64)
            
            # Filter out invalid labels
            valid_mask = gt_labels_3d >= 0
            ...
            if gt_velocity is not None and hasattr(self, 'with_velocity') and self.with_velocity:
            ...
            anns_results = dict(
                gt_bboxes_3d=LiDARInstance3DBoxes(gt_boxes, box_dim=gt_boxes.shape[-1]),
                gt_labels_3d=gt_labels_3d,
                bbox_3d_isvalid=valid_mask
            )
            
            return anns_results
        else:
            # Fall back to parent implementation for new format
    ...
    def union2one(self, queue):
        ...
        """
        # Handle both old format (img, img_metas) and new format (inputs, data_samples)
        def get_img(each):
            # New MMDet3D 2.x format from Pack3DDetInputs
            if 'inputs' in each:
                inputs = each['inputs']
                if isinstance(inputs, dict):
                    if 'img' in inputs:
                        return inputs['img']
                    # Fallback for any legacy key naming
                    if 'imgs' in inputs:
                        return inputs['imgs']
            # Old BEVFormer format with top-level 'img'
            if 'img' in each:
                return each['img'].data if hasattr(each['img'], 'data') else each['img']
            # If neither is present, raise for easier debugging
            raise KeyError(f"Cannot find img in sample. Keys: {each.keys()}")
        
        def get_metas(each):
            if 'data_samples' in each:
                # Extract metainfo from data_samples
                ds = each['data_samples']
                if hasattr(ds, 'metainfo'):
                    return ds.metainfo
                return {}
            elif 'img_metas' in each:
                return each['img_metas'].data if hasattr(each['img_metas'], 'data') else each['img_metas']
            else:
                return {}
        
        imgs_list = [get_img(each) for each in queue]
        metas_map = {}
        prev_scene_token = None
        prev_pos = None
        prev_angle = None
        for i, each in enumerate(queue):
            metas_map[i] = get_metas(each)
            if metas_map[i].get('scene_token') != prev_scene_token:
                metas_map[i]['prev_bev_exists'] = False
                prev_scene_token = metas_map[i].get('scene_token')
                prev_pos = copy.deepcopy(metas_map[i].get('can_bus', np.zeros(18))[:3])
                prev_angle = copy.deepcopy(metas_map[i].get('can_bus', np.zeros(18))[-1])
                if 'can_bus' in metas_map[i]:
                    metas_map[i]['can_bus'][:3] = 0
                    metas_map[i]['can_bus'][-1] = 0
            else:
                metas_map[i]['prev_bev_exists'] = True
                tmp_pos = copy.deepcopy(metas_map[i].get('can_bus', np.zeros(18))[:3])
                tmp_angle = copy.deepcopy(metas_map[i].get('can_bus', np.zeros(18))[-1])
                if 'can_bus' in metas_map[i]:
        ...
        result = queue[-1].copy() if isinstance(queue[-1], dict) else {}
    ...
    def get_data_info(self, index):
        ...
        if len(self.data_infos) == 0:
            return None
        index = max(0, min(index, len(self.data_infos) - 1))
        ...
        if self.modality['use_camera']:
            ...
            input_dict["images"] = {}  # MMDet3D 2.0 expects this
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                # Add to images dict for MMDet3D 2.0
                input_dict["images"][cam_type] = {
                    'img_path': cam_info['data_path'],
                    'cam2img': viewpad,
                    'lidar2cam': lidar2cam_rt_t,
                }
        ...
        if not self.test_mode:
            # Get annotations directly from info instead of calling get_ann_info
            # to avoid infinite recursion (get_ann_info calls get_data_info)
            annos = self.parse_ann_info(info)
            # Add both to ann_info (for LoadAnnotations3D) and directly (for compatibility)
            input_dict['ann_info'] = annos
            # Also add directly so they're available even if LoadAnnotations3D doesn't run
    ...
    def _rand_another(self, idx):
        """Get another random index when the current sample is invalid."""
        import random
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if data is None:
                ...
                retry_count += 1
                if retry_count > 100:
                    raise RuntimeError(f"Failed to load valid data after {retry_count} retries")
                continue
            return data
    def _format_gt(self):
        gt_annos = []
