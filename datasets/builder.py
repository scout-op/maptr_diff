...
# GroupSampler removed in MMDetection 3.x, use local implementation
try:
    from mmdet.datasets.samplers import GroupSampler
except ImportError:
    # Fallback: Use DistributedGroupSampler for non-distributed case
...
def build_dataloader(dataset,
    ...
    if dist:
        ...
        if GroupSampler is not None:
            sampler = GroupSampler(dataset, samples_per_gpu) if shuffle else None
        else:
            # MMDetection 3.x: GroupSampler removed, use None for non-distributed
            print('WARNING: GroupSampler not available in MMDetection 3.x, using default sampler')
            sampler = None
