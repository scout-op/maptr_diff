...
class MapTRAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        # Ensure all GT tensors are on the same device as predictions
        ...
        # Wrap tensors in InstanceData for MMDet 3.x cost function API
        pred_instances = InstanceData(scores=cls_pred, bboxes=bbox_pred[:, :4])
        gt_instances = InstanceData(labels=gt_labels)
        cls_cost = self.cls_cost(pred_instances, gt_instances)
        # regression L1 cost - compute directly to avoid img_meta dependency
        
        normalized_gt_bboxes = normalize_2d_bbox(gt_bboxes, self.pc_range)
        # Compute L1 cost directly (bboxes are already normalized in BEV space)
        reg_cost = torch.cdist(bbox_pred[:, :4], normalized_gt_bboxes[:, :4], p=1)
        reg_cost = reg_cost * self.reg_cost.weight
        ...
        # Compute IoU cost directly to avoid img_meta dependency
        # Since iou_cost.weight is typically 0.0 in MapTR, we can compute a simple version
        if self.iou_cost.weight > 0:
            from mmdet.structures.bbox import bbox_overlaps
            overlaps = bbox_overlaps(bboxes, gt_bboxes, mode=self.iou_cost.iou_mode, is_aligned=False)
            iou_cost = -overlaps * self.iou_cost.weight
        else:
            iou_cost = bboxes.new_zeros((num_bboxes, num_gts))
