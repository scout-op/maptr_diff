...
# Try to import bev_pool from mmdet3d.ops, fall back to PyTorch implementation
try:
    from mmdet3d.ops import bev_pool
except ImportError:
    warnings.warn(
        "mmdet3d.ops.bev_pool not found. Using PyTorch fallback implementation. "
        "This may be slower than the CUDA version."
    )
    
    def bev_pool(feats, coords, B, D, H, W):
        """PyTorch fallback implementation of bev_pool.
        
        Args:
            feats: (N, C) features to pool
            coords: (N, 4) coordinates [x, y, z, batch_idx]
            B: batch size
            D: depth dimension
            H: height dimension  
            W: width dimension
        ...
        Returns:
            (B, C, D, H, W) pooled BEV features
        """
        C = feats.shape[1]
        device = feats.device
        
        # Initialize output tensor
        output = torch.zeros((B, C, int(D), int(H), int(W)), device=device, dtype=feats.dtype)
        ...
        # Scatter add features to BEV grid
        # This is a simple implementation - the CUDA version is more optimized
        for i in range(feats.shape[0]):
            b, xi, yi, zi = batch_idx[i], x[i], y[i], z[i]
            if 0 <= xi < H and 0 <= yi < W and 0 <= zi < D:
                output[b, :, zi, xi, yi] += feats[i]
